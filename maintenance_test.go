...
func Test_convertUTCDayAndHourToTimezone(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			givenHour:        4,
			givenLocation:    time.FixedZone("UTC-8", -8*60*60),
	...
	for _, testCase := range testCases {
		t.Run(fmt.Sprintf("converts a weekday / hour from UTC to specific timezone (%s)", testCase.givenLocation.String()), func(t *testing.T) {
			// Given
			weekDay := testCase.givenWeekDay
			hour := testCase.givenHour
			location := testCase.givenLocation

			// When
			resultWeekDay, resultHour := convertUTCDayAndHourToTimezone(weekDay, hour, location)
...
func TestMaintenanceWindow_formatMaintenanceWindowWithTimezone(t *testing.T) {
	t.Run("it formats the maintenance window in specified timezone", func(t *testing.T) {
		...
		// When
		result := maintenanceWindow.formatMaintenanceWindowWithTimezone(location)
