...
func TestAlertsClient_Show(t *testing.T) {
	appName := "my-app"
	alertID := "my-id"
	runs := map[string]struct {
		...
		"it should fail if it fails to get the subresource": {
			expectMockReturnCall: func(call *gomock.Call) {
				call.Return(errors.New("error get"))
			},
			expectedError: "error get",
		},
		"it should succeed if it succeeds to get the subresource": {
			expectMockReturnCall: func(call *gomock.Call) {
				call.Return(nil)
			},
		},
	...
	for msg, run := range runs {
		t.Run(msg, func(t *testing.T) {
			...
			run.expectMockReturnCall(mock.EXPECT().subresourceGet(appName, "alerts", alertID, gomock.Any(), gomock.Any()))
...
func TestAlertsClient_Update(t *testing.T) {
	appName := "my-app"
	alertID := "my-id"
	runs := map[string]struct {
		expectMockReturnCall func(call *gomock.Call)
		...
		"it should fail if it fails to update the subresource": {
			expectMockReturnCall: func(call *gomock.Call) {
				call.Return(errors.New("error update"))
			},
			expectedError: "error update",
		},
		"it should succeed if it succeeds to update the subresource": {
			expectMockReturnCall: func(call *gomock.Call) {
				call.Return(nil)
			},
		},
	...
	for msg, run := range runs {
		t.Run(msg, func(t *testing.T) {
			...
			run.expectMockReturnCall(mock.EXPECT().subresourceUpdate(appName, "alerts", alertID, gomock.Any(), gomock.Any()))
